...
export function CTA({
  title = "Redo för en bil som känns ny igen?",
  text = "Boka premium bilvård i Karlskrona med trygg garanti och över 20 års erfarenhet."
}: {
  title?: string;
  text?: string;
}) {
    ...
    <section className="section-padding">
      <div className="container-xl overflow-hidden rounded-[32px] border border-white/10 bg-carbon p-8 text-white shadow-2xl md:p-12">
        ...
        <div className="grid gap-8 md:grid-cols-[1fr_auto] md:items-center">
          <div>
            <p className="eyebrow text-zinc-300">Vikings Car Care</p>
            <h2 className="mt-3 text-3xl font-black md:text-5xl">{title}</h2>
            <p className="mt-4 max-w-2xl leading-8 text-zinc-300">{text}</p>
          </div>
